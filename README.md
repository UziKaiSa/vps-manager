...
1. 是否开启 BBR
2. 是否配置 SSH 高位端口和仅密钥登录

这两个操作可以分别跳过。选择初始化并不代表必须同时修改 BBR 和 SSH。

### SSH 加固注意事项

SSH 管理用户自动使用当前运行脚本的登录用户，不需要另外填写用户名。
...
```text
1) 显示 Windows PowerShell 生成 Ed25519 密钥的命令
2) 显示 Linux/macOS 生成 Ed25519 密钥的命令
3) 校验公钥并显示指纹
4) 查看当前管理用户的 authorized_keys
```

它只生成客户端命令或读取公钥，不会在 VPS 上生成、保存或显示客户端私钥。
...
更新脚本本身不会自动覆盖现有 Xray、SSH、YAML 或 Komari 配置。
